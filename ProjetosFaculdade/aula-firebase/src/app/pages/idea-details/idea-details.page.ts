...
export class IdeaDetailsPage implements OnInit {
  ...
  ionViewWillEnter(){
    ...
    if(id){
      this.ideaService.getIdea(id).subscribe(data => this.idea = data)
    }
  }
  ...
  addIdea(){
    ...
    }, err => this.showToast('Add deu ruim!')
    )
  }
  ...
  deleteIdea(){
    ...
    }, err => this.showToast('Delete deu ruim!')
    )
  }
  ...
  updateIdea(){
    ...
    }, err => this.showToast('Update deu ruim!')
    )
  }
